...
{
    return sample_points<double, 3u>(argc,argv,points_normals,iXForm,weight_samples);
}
...
MESH_D poisson_reconstruction_d(int argc, char* argv[], const POINTS_NORMALS_D& points_normals, const std::vector<double>* weight_samples)
{
    ...
    //controller.P();
    MESH_D res =  poisson_reconstruction<double, 3u>(argc, argv, points_normals, weight_samples);
    //controller.V();
    return res;
}
